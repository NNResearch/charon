...
static std::map<Networks::Nets, Network> networks = {
    {Networks::Nets::ACAS_XU, read_network(CHARON_HOME + std::string("example/acas_xu_1_1.txt"))}
};
...
int main(int argc, char** argv) {
    std::cout << "start training using bayesian network.\n";
    if (argc != 2) {
    ...
    for (const auto e : Networks::All ) {
        ...
        networkAttacks[e] = Attack_For_Net(net);
    }
