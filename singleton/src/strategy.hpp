...
class StrategyInterpretation {
    public:
        ...
        int size() {
            return domain_input_size() * domain_output_size() + split_input_size() * split_output_size();
        }
        ...
        template <class vectype>
            void vec_to_strategy_mats(const vectype& vec, Mat& domain_strat, Mat& split_strat) const {
                int dis, dos, sis, sos, dim;
                fill_strategy_size(dis, dos, sis, sos, dim);
                domain_strat.resize(dos, dis);
                for (int i = 0; i < dos * dis; i++) {
                    domain_strat(i / dis, i % dis) = vec(i);
                }
                split_strat.resize(sos, sis);
                for (int i = 0; i < sos * sis; i++) {
                    split_strat(i / sis, i % sis) = vec(dos * dis + i);
                }
            }
        /**
